package com.drugstore.server.repository;

import com.drugstore.server.domain.Drug;
import com.drugstore.server.domain.Prescription;
import com.drugstore.server.domain.PrescriptionType;

import java.util.List;

/**
 * Created by IntelliJ IDEA.
 * User: ekisa
 * Date: 20.11.2011
 * Time: 22:54
 * To change this template use File | Settings | File Templates.
 */
public interface PrescriptionRepository extends EntityRepository<Prescription>{
    List<Drug> listAllDrugsWithThisPrescriptionType(PrescriptionType prescriptionType);
}
